...
if iscell(x)
    ...
    for i = 1:length(obj.dims)
        dx{i} = dynamics_cell_helper(obj, x, u, obj.dims(i), q_mode);
    end
    ...
    switch q_mode
        ...
        dx(3) = u_num;
        case 2
        dx(1) = 0.5 * obj.v * cos(x(3));
        dx(2) = 0.5 * obj.v * sin(x(3));
        dx(3) = u_num;
    end
...
%%
function dx = dynamics_cell_helper(obj, x, u, dim, q_mode)
...
switch q_mode
    case 1
        switch dim
        case 1
        dx = 2 * obj.v * cos(x{obj.dims==3}).*is_in_obst(x,obj.obst_range);
        case 2
        dx = 2 * obj.v * sin(x{obj.dims==3}).*is_in_obst(x,obj.obst_range);
        case 3
        dx = u;
        end
    case 2
        switch dim
        case 1
        dx = 0.5 * obj.v * cos(x{obj.dims==3});
        case 2
        dx = 0.5 * obj.v * sin(x{obj.dims==3});
        case 3
        dx = u;
        end
end
